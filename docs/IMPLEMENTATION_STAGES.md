...
| 4S | Cross-cutting cryptographic qualification and key-custody evidence: threat/policy revisions, provider states, typed key metadata, cryptoperiod/lifecycle/succession records, immutable recovery, native provider-registration inventory and bounded provider-open observation | IN PROGRESS | Security, custody, inventory and provider-open tests pass; the Software KSP opened for a provider-level property query while the registered Platform/TPM KSP truthfully recorded `NTE_DEVICE_NOT_READY`; no key or cryptographic operation ran, and authentication, authorization, independent qualification and security review remain closed |

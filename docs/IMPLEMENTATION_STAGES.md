...
| 4S | Cross-cutting cryptographic qualification and key-custody evidence: threat/policy revisions, provider states, typed key metadata, cryptoperiod/lifecycle/succession records, immutable recovery and read-only native provider-registration inventory | IN PROGRESS | `genesis_security_tests`, `genesis_key_custody_tests` and `genesis_platform_crypto_inventory_tests` pass; Windows CNG registration was observed without opening a provider; actual provider/key operations, authentication, authorization, security review and platform qualification remain closed |

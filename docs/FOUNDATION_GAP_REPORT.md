...
- The 0.28 native cryptographic platform inventory makes one read-only Windows
  CNG provider-registration query, canonicalizes bounded UTF-8 names, records
  elapsed observation evidence, and retains provider-open, key-access,
  operation and qualification claims as false. The local Windows x86-64 run
  observed four registrations in 428 microseconds; it did not access a provider
  or key. Non-Windows builds return an explicit unsupported record. See
  `docs/security/CRYPTO_PLATFORM_INVENTORY.md`.
- The 0.29 provider-open observation requires an exact match in a verified
  registration inventory, opens only that Windows KSP, queries only its
  provider-level implementation type with UI suppression, and releases its
  handle. The Software KSP completed that route and reported the software flag;
  the registered Platform/TPM KSP truthfully retained `NTE_DEVICE_NOT_READY`.
  Both records permanently deny all key, operation and qualification claims.
  See `docs/security/CRYPTO_PROVIDER_OPEN_OBSERVATION.md`.
- Tests cover SHA-256 vectors, deterministic runtime and replay boundaries, registry/schema checks, provenance chaining, identity, typed entities and relations, life-record birth/name/reference/continuity invariants, immutable recovery, genome, RNA, maturity, lineage adversaries, deterministic births, mutation records, inheritance cutoff boundaries, organism composition, signalling, metabolism, homeostasis, immunity, memory, learning, affect, perception, provider inventory, and bounded provider-open state and platform routes.
...
5. The 0.26 qualification registry, 0.27 key-custody metadata layer, 0.28 read-only Windows CNG inventory and 0.29 bounded provider-open route now record threat/policy/provider evidence, secret-free external-handle bindings, lifecycle/recovery evidence, registered KSP names, open availability, implementation-type flags and native failures. They deliberately perform no cryptography; registration, successful open and provider-reported flags are not qualification. Next independently qualify an exact module and operational environment; implement native non-exportable key generation/open/use/deletion plus authentication and authorization; then add authenticated migration/recovery, fuzzing, sanitizer qualification, coverage thresholds, external review and platform/device/model matrices. Current hashes are deterministic integrity evidence, not authentication.

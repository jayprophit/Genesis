...
5. The 0.26 qualification registry, 0.27 key-custody metadata layer and 0.28 read-only Windows CNG inventory now record threat/policy/provider evidence, secret-free external-handle bindings, lifecycle/recovery evidence and registered KSP names. They deliberately perform no cryptography and registration is not qualification. Next observe a bounded provider-open route and independently qualify its exact module and operational environment; implement native non-exportable key generation/open/use/deletion plus authentication and authorization; then add authenticated migration/recovery, fuzzing, sanitizer qualification, coverage thresholds, external review and platform/device/model matrices. Current hashes are deterministic integrity evidence, not authentication.
